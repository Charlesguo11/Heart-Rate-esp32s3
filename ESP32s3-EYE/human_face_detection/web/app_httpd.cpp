...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    if (res == ESP_OK)
    {
        ...
        ESP_LOGI(TAG, "server scocket created");
        /*监听连接请求--监听队列长度为1*/  
        ...
        int frameLength = int(_jpg_buf_len);
        ESP_LOGI(TAG, "sending pic size %d",frameLength);  
        char lenStr[128];
        itoa(_jpg_buf_len,lenStr,10);
        send(clientSocket,lenStr,strlen(lenStr),0);
        char flag[1];
        while(1){
            recv(clientSocket,flag,1,0);
            if(isalpha(flag[0])) break;
        }
        ESP_LOGI(TAG,"size was recived");  
        ESP_LOGI(TAG,"sending pic");  
        send(clientSocket,(char*)_jpg_buf,_jpg_buf_len*8,0);
        ESP_LOGI(TAG, "all recieved");  
        close(serveSocket);
        close(clientSocket);
    }
